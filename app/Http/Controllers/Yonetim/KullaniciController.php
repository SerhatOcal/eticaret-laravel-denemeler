...
class KullaniciController extends Controller
{
    ...
    public function oturumac()
    {
        if (request()->isMethod('POST'))
        {
            ...
            if (Auth::guard('yonetim')->attempt($oturumbilgileri, request()->has('benihatirla')))
            {
                return redirect()->route('yonetim.anasayfa');
            }
            else {
                return back()->withInput()->withErrors(['email' => 'Giriş Bilgileri Hatalı!']);
            }
        }
    ...
    public function index()
    {
        if (request()->filled('aranan')){
            ...
            $kullanicilar = Kullanici::orderByDesc('olusturulma_tarihi')->paginate(8);
        }
